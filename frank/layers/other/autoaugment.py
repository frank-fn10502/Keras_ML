...
def distort_image_with_autoaugment(image, augmentation_name):
  ...
  augmentation_params = Config(cutout_const=100, translate_const=250)
  ...
  for layer_num in range(num_layers):
    ...
    with tf.name_scope('randaug_layer_{}'.format(layer_num)):
      for (i, op_name) in enumerate(available_ops):
        ...
        func, _, args = _parse_policy_info(op_name, prob, random_magnitude, replace_value, augmentation_params)
        image = tf.cond(
              tf.equal(i, op_to_select),
              lambda selected_func=func, selected_args=args: tf.cast( selected_func( tf.cast(image ,tf.uint8), *selected_args), tf.float32 ),
              lambda: image
            )
  return image
