...
class DistortImage(layers.Layer):
    ...
    def __init__(self, totalEpoch=0):
        ...
        self.layer_resizing = layers.Resizing(
            *self.imgSize, interpolation='nearest')

    def setStages(self, n):
        self.__stages = n
        self.__numPerStage = self.totalEpoch // self.__stages
        self.__magnitudePerStage = (
            self.__maxMagnitude - self.__minMagnitude) / self.__stages

        self.__imgSizePerStage = (
            self.__maxImgSize[0] - self.__minImgSize[0]) / self.__stages

    def setNewMagnitude(self, currentEpoch):
        # (得到現在的 stage) * 每一個 stage 的增長數量 + 基礎的值
        self.magnitude = (currentEpoch // self.__numPerStage) * \
...
class InceptionBlock(layers.Layer):
    ...
    def _build(self, depth):
        ...
        def getConv(k, f):
            return layers.Conv2D(kernel_size=k, filters=f, padding='same', activation='relu')
...
class ResNet50_BN_Conv(layers.Layer):
    def __init__(self, filters, kernel_size=(3, 3), stride=1):
        ...
        self.conv = layers.Conv2D(
            kernel_size=kernel_size, filters=filters, strides=stride, padding='same')
...
class ResidualBottleneckBlock(layers.Layer):
    ...
    def _build(self, f):
        ...
        self.conv1 = ResNet50_BN_Conv(kernel_size=(
            1, 1), filters=cf1, stride=2 if self.needDownSample else 1)
        self.conv2 = ResNet50_BN_Conv(filters=cf2)
        self.conv3 = ResNet50_BN_Conv(kernel_size=(1, 1), filters=cf3)

        self.reshapeConv = ResNet50_BN_Conv(kernel_size=(
            1, 1), filters=cf3, stride=2 if self.needDownSample else 1)
